var werkHelper = {
    parse: function(werk){
        ...
        var xMax = d3.max(werk.data,function(series){return d3.max(series.values,function(d){return d.x; }); }),
	        ...
	        yMax = d3.max(chartwerk.data,function(d){
	            return d3.sum(chartwerk.datamap.series,function(series){
	                    return d[series];
	            });
	        }),
	        yMin = d3.min(chartwerk.data,function(d){
	            return d3.sum(chartwerk.datamap.series,function(series){
	                    return d[series];
	            });
	        });
    ...
    valueDomain: function(werk){
        var yMax = d3.max(chartwerk.data,function(d){
	            return d3.sum(chartwerk.datamap.series,function(series){
	                    return d[series];
	            });
	        }),
