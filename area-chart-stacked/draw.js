function draw(){
    ...
    var stack = d3.stack();
    stack.keys(chartwerk.datamap.series);
    stack.order(d3.stackOrderDescending);
    
    // console.log(werk)
    // console.log(stack(werk.data))
    // console.log(stack(chartwerk.data))
    var area = d3.area()
        .x(function(d) { return werk.scales.x(d3.timeParse(d3.timeParse( chartwerk.axes.base.dateFormat ))(String(d.data[chartwerk.datamap.base]))); })
        .y0(function(d) { return werk.scales.y(d[0]); })
        .y1(function(d) { return werk.scales.y(d[1]); });
    ...
    series.append("path")
    		...
    		.style("fill", function(d,i){ return werk.scales.color(chartwerk.datamap.series[i]);})
    		.style("stroke","none");
    ...
    function showTooltip(){
	    ...
	    trackLine
			...
			.attr("x2", werk.scales.x(werk.parsers.base(
                            String(nearestX[chartwerk.datamap.base]))))
			.attr("y2", werk.scales.y(d3.sum(chartwerk.datamap.series,function(d){ return nearestX[d]; })))
			.style("stroke","#666")
        ...
        tooltip.selectAll(".value")
          ...
          .sort(function(a,b) { return d3.sum(chartwerk.data,function(d){ return d[a]; }) - d3.sum(chartwerk.data,function(d){ return d[b]; }); });
          
        tooltip.select(".total")
          .text(function(){
            var v = chartwerk.axes.value;
            return "Total: " + v.prefix + dollar(d3.sum(chartwerk.datamap.series,function(series) { return nearestX[series]; })) + v.suffix; 
          });
          
        var p = d3.mouse(this.parentElement.parentElement);
        d3.select(".tooltip")
            .style("opacity", 1)
            .style("top", werk.scales.y(d3.sum(chartwerk.datamap.series,function(d){ return nearestX[d]; })) + "px")
            .style("left", function(){
